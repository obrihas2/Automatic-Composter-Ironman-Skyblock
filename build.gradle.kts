...
loom {
    launchConfigs {
        "client" {
            ...
            // Suppress websocket spam from other mods
            property("log4j2.configurationFile", "log4j2.xml")
...
tasks.withType(JavaCompile::class) {
    ...
    options.isFork = true
    options.forkOptions.jvmArgs?.addAll(listOf("-Xmx3g", "-XX:+UseG1GC", "-XX:+UseStringDeduplication"))
    options.compilerArgs.addAll(listOf("-Xlint:-deprecation", "-Xlint:-unchecked"))
    // Use multiple compiler threads
...
tasks.shadowJar {
    ...
    // Removed debug output for faster builds
    // doLast {
...
// Use JUnit Platform for tests; if tests are not relevant for CI builds, this can be toggled off.
tasks.test {
    useJUnitPlatform()
    // Disable tests by default for mod builds; re-enable locally if needed.
    enabled = false
}
